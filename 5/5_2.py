...
for instruction_num, instruction_values in instructions_dict.items():
    ...
    if instruction_values['start_x'] - instruction_values['end_x'] == 0:
        ...
        move_list = list(zip(range(instruction_values['start_x'], instruction_values['end_x'] + x_step, x_step),
                             range(instruction_values['start_y'], instruction_values['end_y'] + y_step, y_step)))

        for x_spot, y_spot in move_list:
            sea_map[x_spot][y_spot] += 1
