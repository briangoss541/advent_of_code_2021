...
for pulled_number in pulled_numbers:
    ...
    if winning_number is not None and winning_board is not None:
        break

print(winning_board, winning_number)
print(board_data[winning_board])

# Summing unmarked from winning board
winning_board_sum = 0

for x_value, target_row in enumerate(board_data[winning_board]):
    for y_value, target_col in enumerate(target_row):
        if (x_value, y_value) not in hits_dict[winning_board]:
